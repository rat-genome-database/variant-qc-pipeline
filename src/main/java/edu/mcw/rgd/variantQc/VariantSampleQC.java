...
public class VariantSampleQC {
    ...
    void main() throws Exception {
        ...
        File folder = new File(qcFolder);
        ArrayList<File> files = new ArrayList<>();
        dao.listFilesInFolder(folder, files);

        for (File file : files){
            try (BufferedReader br = openFile(file.getPath())) {
                String lineData;
                String[] cols = {};
...
//                                logger.info("Variant not found!");
                        ...
                        for (int i = 9; i < lineSplit.length; i++) {
                            ...
                            if (!variantMap.isEmpty()) {
                                for (Integer varKey : varKeys) {
                                    ...
                                    if (updateMe) {
                                        zygosity.computeZygosityStatus(vsd.getVariantFrequency(),vsd.getDepth(), s.getGender(), v, vsd);
                                        tobeUpdated.add(vsd);
                                    }
                                ...
                                if (updateMe) {
                                    zygosity.computeZygosityStatus(vsd.getVariantFrequency(),vsd.getDepth(), s.getGender(), var, vsd);
                                    tobeUpdated.add(vsd);
                                }
                            }
                        } // end strains for
    ...
    public String getQcFolder() {
        return qcFolder;
    }

    public void setQcFolder(String qcFolder) {
